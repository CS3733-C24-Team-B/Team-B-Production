...
export default function ServiceRequestTable() {
  ...
  const [sortFunction, setSortFunction] = useState<
    (srA: ServiceRequestWithTypes, srB: ServiceRequestWithTypes) => number
  >(() => () => {
    return 0;
  });
  const [typeSort, setTypeSort] =
    useState<keyof typeof requestSortField>("timeCreated");
  ...
  function sqlToDate(sqlDate: string) {
    ...
    const sMillisecond = parseInt(
      sqlDateArrSecs[1].substring(0, sqlDateArrSecs[1].length - 1),
    );
  ...
  if (!beingSorted) {
    srData.sort(
      (srA: ServiceRequestWithTypes, srB: ServiceRequestWithTypes) => {
        return (
          sqlToDate(srA.timeCreated.toString()).getTime() -
          sqlToDate(srB.timeCreated.toString()).getTime()
        );
      },
    );
  } else {
  ...
  function sortByPrio(
    srA: ServiceRequestWithTypes,
    srB: ServiceRequestWithTypes,
  ) {
    return (
      prioSort[srA.priority as keyof typeof prioSort] -
      prioSort[srB.priority as keyof typeof prioSort]
    );
  }
  ...
  function sortByStatus(
    ...
    return (
      statusSort[srA.status as keyof typeof statusSort] -
      statusSort[srB.status as keyof typeof statusSort]
    );
  }
  ...
  function sortRequests(sortField: requestSortField) {
    switch (sortField) {
      ...
      case requestSortField.timeCreated:
        setSortFunction(
          () => (a: ServiceRequestWithTypes, b: ServiceRequestWithTypes) =>
            sqlToDate(a.timeCreated.toString()).getTime() -
            sqlToDate(b.timeCreated.toString()).getTime(),
        );
        break;
      case requestSortField.type:
        setSortFunction(
          () => (a: ServiceRequestWithTypes, b: ServiceRequestWithTypes) =>
            getReqType(a).localeCompare(getReqType(b)),
        );
        break;
      case requestSortField.assignedTo:
        setSortFunction(
          () => (a: ServiceRequestWithTypes, b: ServiceRequestWithTypes) => {
            ...
            return a.assignedTo.firstName!.localeCompare(
      ...
      case requestSortField.location:
        setSortFunction(
          () => (a: ServiceRequestWithTypes, b: ServiceRequestWithTypes) =>
            nodeNameOrReturn(a.locationID as string).localeCompare(
              nodeNameOrReturn(b.locationID as string),
            ),
        );
        break;
    ...
    return (
      <>
        <TableRow>
          ...
          <TableCell>
            {nsr.assignedID
              ? nsr.assignedTo.firstName + " " + nsr.assignedTo.lastName
              : ""}
          </TableCell>
          <TableCell>
          ...
          <TableCell>
            {nsr.createdBy
              ? nsr.createdBy.firstName + " " + nsr.createdBy.lastName
              : ""}
          </TableCell>
          <TableCell>
            {sqlToDate(nsr.timeCreated.toString()).getMonth() +
              "/" +
              sqlToDate(nsr.timeCreated.toString()).getDate() +
              "/" +
              sqlToDate(nsr.timeCreated.toString()).getFullYear() +
              "\n" +
              sqlToDate(nsr.timeCreated.toString()).getHours() +
              ":" +
              sqlToDate(nsr.timeCreated.toString())
                .getMinutes()
          ...
          <TableRow>
            ...
            <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={8}>
              <Collapse in={open} timeout="auto" unmountOnExit>
                <Box sx={{ margin: 1 }}>
                  {Object.keys(nsr[getReqType(nsr) as keyof typeof getReqType])
                    .filter((key) => {
                      console.log("KEY: " + key);
                    ...
                    .map((key) => (
                      <p>
                        {RequestSpecifics[
                          key as keyof typeof RequestSpecifics
                        ] +
                          nodeNameOrReturn(
                            nsr[getReqType(nsr) as keyof typeof getReqType][
                              key
                            ],
                          )}
                      </p>
                ...
                <>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      setFilterFunction(
                        () => (nsr: ServiceRequestWithTypes) => {
                          return (
                            e.target.value === "Choose Type" ||
                            getReqType(nsr) === e.target.value
                          );
                        },
                      );
                    }}
                  >
                    ...
                    <MenuItem value={"internalTransport"}>
                      Internal Transport
                    </MenuItem>
              ...
              Status: (
                <>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      setFilterFunction(
                        () => (nsr: ServiceRequestWithTypes) => {
                          return (
                            e.target.value === "Choose Status" ||
                            nsr.status === e.target.value
                          );
                        },
                      );
                    }}
                  >
                    ...
                    <MenuItem value={StatusType.Unassigned}>
                      Unassigned
                    </MenuItem>
                    <MenuItem value={StatusType.Assigned}>Assigned</MenuItem>
                    <MenuItem value={StatusType.InProgress}>
                      In Progress
                    </MenuItem>
              ...
              Priority: (
                <>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      setFilterFunction(
                        () => (nsr: ServiceRequestWithTypes) => {
                          return (
                            e.target.value === "Choose Priority" ||
                            nsr.priority === e.target.value
                          );
                        },
                      );
                    }}
                  >
                    ...
                    <MenuItem value={PriorityType.Emergency}>
                      Emergency
                    </MenuItem>
                  </Select>
                </>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      setFilterFunction(
                        () => (nsr: ServiceRequestWithTypes) => {
                          return (
                            e.target.value === "Choose Employee" ||
                            nsr.assignedID === e.target.value
                          );
                        },
                      );
                    }}
                ...
                <TableRow>
                  <TableCell>
                    <IconButton
                      onClick={(e) => {
                        setMenuAnchor(
                          e.currentTarget as unknown as SetStateAction<null>,
                        );
                      }}
                  ...
                  <TableCell sx={{ width: 100, padding: 0 }}>
                    ...
                    <IconButton
                      style={{
                        color: typeSort === "priority" ? "#34AD84" : "",
                        width: "2vw",
                      }}
                      onClick={() => {
                        setSortUp(!sortUp);
                  ...
                  <TableCell>
                    ...
                    <IconButton
                      style={{
                        color: typeSort === "location" ? "#34AD84" : "",
                        width: "2vw",
                      }}
                      onClick={() => {
                        setSortUp(!sortUp);
                  ...
                  <TableCell>
                    ...
                    <IconButton
                      style={{
                        color: typeSort === "assignedTo" ? "#34AD84" : "",
                        width: "2vw",
                      }}
                      onClick={() => {
                        setSortUp(!sortUp);
                  ...
                  <TableCell>
                    ...
                    <IconButton
                      style={{
                        color: typeSort === "createdBy" ? "#34AD84" : "",
                        width: "2vw",
                      }}
                      onClick={() => {
                        setSortUp(!sortUp);
                  ...
                  <TableCell>
                    ...
                    <IconButton
                      style={{
                        color: typeSort === "timeCreated" ? "#34AD84" : "",
                        width: "2vw",
                      }}
                      onClick={() => {
                        setSortUp(!sortUp);
                        ...
                        {
                          {
                            ...
                            Priority:
                              priorityFilter + " priority service requests",
                            Employee:
                              "service requests assigned to " +
                              (userEmployee &&
