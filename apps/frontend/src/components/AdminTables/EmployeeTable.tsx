...
export default function AdminViewer() {
  ...
  function sortEmployees(sortField: employeeSortField) {
    ...
    switch (sortField) {
      case employeeSortField.email:
        employeesCopy.sort((a: UpdateEmployee, b: UpdateEmployee) =>
  ...
  const employeeList = employees.map(
    (employee: UpdateEmployee, index: number) => {
      return (
        <TableRow>
          ...
          <TableCell>
            <Button
              ...
              style={{
                color:
                  employee.email === "softengc24b@gmail.com"
                    ? "grey"
                    : "#012D5A",
              }}
            ...
            <DialogActions>
              <Button
                onClick={() => {
                  ...
                  getAccessTokenSilently().then((accessToken: string) => {
                    axios
                      .delete("/api/employee/" + employee.email, {
                        headers: {
                          Authorization: "Bearer " + accessToken,
                        },
                      })
