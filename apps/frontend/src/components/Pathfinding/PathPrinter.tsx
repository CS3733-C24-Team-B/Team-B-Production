...
function determineTurnDirection(
  previousNode: string,
  nodeStart: string,
  nodeEnd: string,
): string {
  ...
  const x = parseInt(
    previousNode.substring(
      previousNode.indexOf(":") + 1,
      previousNode.lastIndexOf(":"),
    ),
  );
  const y = parseInt(previousNode.substring(previousNode.lastIndexOf(":") + 1));
  const x1 = parseInt(
    nodeStart.substring(nodeStart.indexOf(":") + 1, nodeStart.lastIndexOf(":")),
  );
  const y1 = parseInt(nodeStart.substring(nodeStart.lastIndexOf(":") + 1));
  const x2 = parseInt(
    nodeEnd.substring(nodeEnd.indexOf(":") + 1, nodeEnd.lastIndexOf(":")),
  );
...
function directNode(previousNode: string, nodeStart: string, nodeEnd: string) {
  if (previousNode.substring(0, 2) != nodeStart.substring(0, 2)) {
    return (
      "Take " +
      previousNode.substring(2, previousNode.indexOf(":")) +
      " to " +
      nodeStart.substring(0, 2)
    );
  }
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const res2 = await axios.get(
        `/api/path/${data.startNode}/${data.endNode}`,
      );
      ...
      let x: number = parseInt(
        coords[coords.length - 1].substring(
          coords[coords.length - 1].indexOf(":") + 1,
          coords[coords.length - 1].lastIndexOf(":"),
        ),
      );
      let y: number = parseInt(
        coords[coords.length - 1].substring(
          coords[coords.length - 1].lastIndexOf(":") + 1,
        ),
      );
      ...
      for (let i = coords.length - 2; i > 0; i--) {
        ...
        if (direction != "") {
          const currx = parseInt(
            coords[i].substring(
              coords[i].indexOf(":") + 1,
              coords[i].lastIndexOf(":"),
            ),
          );
          const curry = parseInt(
            coords[i].substring(coords[i].lastIndexOf(":") + 1),
          );
      ...
      directionsFloor[levels] = joinedwords;
      joinedwords.push(
        "Starting at " + nodeIDs[0] + " head in the direction of " + nodeIDs[1],
      );
