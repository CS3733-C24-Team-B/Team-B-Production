...
export default function LeafletMap(props: MapProps) {
  ...
  useEffect(() => {
    ...
    if (props.defaultStart === "" || !props.useDefault) {
      ...
      if (props.defaultStart !== undefined && nodeData.length > 0) {
        ...
        (lMap!.current! as L.Map).setView(
          new LatLng(
            transY(nodeIDToYPos(props.defaultStart)),
            transX(nodeIDToXPos(props.defaultStart)),
          ),
          6,
        );
      }
  ...
  useEffect(() => {
    ...
    if (props.zoomNode !== oldZoom) {
      ...
      (lMap!.current! as L.Map).setView(
        new LatLng(
          transY(nodeIDToYPos(props.zoomNode)),
          transX(nodeIDToXPos(props.zoomNode)),
        ),
        8,
      );
  ...
  useEffect(() => {
    async function fetch() {
      if (isAuthenticated) {
        ...
        setSRData(res.data);
        setAnimatePic(
          res2.data ? "data:image;base64," + res2.data : user!.picture!,
        );
      }
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const nodeIDs = res2.data.reduce(
        (
          accumulator: string[],
          roomData: {
            nodeID: string;
  ...
  useEffect(() => {
    ...
    if (showEdges) {
      edgeData.map(({ startNodeID, endNodeID }) => {
        if (
          nodeIDToFloor(startNodeID) === currLevel &&
          nodeIDToFloor(endNodeID) === currLevel
        ) {
        ...
        if (nodeIDToFloor(nr) === currLevel) {
          if (startX >= 0 && startY >= 0) {
            temp.push(
              <Polyline
                positions={[
                  [transY(startY), transX(startX)],
                  [transY(nodeIDToYPos(nr)), transX(nodeIDToXPos(nr))],
                ]}
                color={edgeColor}
                weight={5}
              ></Polyline>,
            ...
            for (let i = 0; i < steps; i++) {
              animate.push(
                new LatLng(
                  transY(startY) + (dy * i) / steps,
                  transX(startX) + (dx * i) / steps,
                ),
              );
            }
          } else if (prevFloor !== "") {
            if (showPopups) {
              temp.push(
                <Popup
                  position={[
                    transY(nodeIDToYPos(nr)),
                    transX(nodeIDToXPos(nr)),
                  ]}
                  autoClose={false}
                  interactive={true}
                >
          ...
          if (startX >= 0 && startY >= 0) {
            if (showPopups) {
              temp.push(
                <Popup
                  position={[transY(startY), transX(startX)]}
                  autoClose={false}
                  interactive={true}
                >
                  <p>
                    {" "}
                    {floorChanges + ". Go to floor "}{" "}
                    <span
                      onClick={() => {
      ...
      return !props.goku ? (
        <ImageOverlay
          ...
          bounds={
            new LatLngBounds(
              new LatLng(
                animateData[startDraw.current].lat - 0.5,
                animateData[startDraw.current].lng - 0.5,
              ),
              new LatLng(
                animateData[startDraw.current].lat + 0.5,
                animateData[startDraw.current].lng + 0.5,
        ...
        <SVGOverlay
          bounds={
            new LatLngBounds(
              new LatLng(
                animateData[startDraw.current].lat - 1,
                animateData[startDraw.current].lng - 1,
              ),
              new LatLng(
                animateData[startDraw.current].lat + 1,
                animateData[startDraw.current].lng + 1,
  ...
  function drawNodeStart() {
    if (
      nodeStart !== "" &&
      nodeData.length > 0 &&
      nodeIDToFloor(nodeStart) === currLevel
    ) {
      return (
        <>
          {showPopups ? (
            <Popup
              position={
                new LatLng(
                  transY(nodeIDToYPos(nodeStart)),
                  transX(nodeIDToXPos(nodeStart)),
                )
              }
              autoClose={false}
            >
              <p>
                Starting Node:{" "}
                <span className={"floor-change-text"}>
                  {nodeIDtoName(nodeStart)}
                </span>
              </p>
            </Popup>
          ) : (
        ...
        <>
          {showPopups ? (
            <Popup
              position={
                new LatLng(
                  transY(nodeIDToYPos(nodeEnd)),
                  transX(nodeIDToXPos(nodeEnd)),
                )
              }
              autoClose={false}
            >
  ...
  function nametoNodeID(name: string) {
    return nodeData.find(({ longName }) => longName === name)!["nodeID"];
  }
  ...
  function closestNodeToLatLng(pos: LatLng) {
    ...
    nodeData.forEach(({ nodeID, xcoord, ycoord, floor }) => {
      if (currLevel === floor) {
        if (
          closestDist === -1 ||
          getDistFromLatLng(pos, transX(xcoord), transY(ycoord)) < closestDist
        ) {
          closestID = nodeID;
    ...
    <div style={{ maxHeight: "100%" }}>
      ...
      >
        <div className="drawer-content">
          <div
            className="drawer-search-bars"
            style={{ marginBottom: "10px", width: "100%", minWidth: 250 }}
          >
            <div className="autocomplete-rows" style={{ width: "100%" }}>
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label={<p className="autocomplete-text">Start Node...</p>}
                  />
                )}
                ...
                onChange={(newValue) => {
                  ...
                  if (input.innerText !== undefined) {
                    ...
                    (lMap!.current! as L.Map).setView(
                      new LatLng(
                        transY(nodeIDToYPos(nId)),
                        transX(nodeIDToXPos(nId)),
                      ),
                      8,
                    );
                  } else {
            ...
            >
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    label={<p className="autocomplete-text">End Node...</p>}
                  />
                )}
      ...
      >
        ...
        />
        {nodeData.map(
          ({ nodeID, longName, xcoord, ycoord, floor, nodeType }) =>
            floor === currLevel &&
            showNodes &&
            (showHalls || nodeType !== "HALL") ? (
              showIcons ? (
                <SVGOverlay
                  interactive={true}
                  bounds={
                    new LatLngBounds(
                      new LatLng(
                        34.8 - (ycoord * 34) / 3400 - 0.25,
                        (xcoord * 50) / 5000 + 3 - 0.25,
                      ),
                      new LatLng(
                        34.8 - (ycoord * 34) / 3400 + 0.25,
                        (xcoord * 50) / 5000 + 3 + 0.25,
                        ...
                        <div key={serviceRequest.serviceID}>
                          {serviceRequest.locationID === nodeID && (
                            <div>
                              <Divider />
                              <p>
                                Contains {getReqType(serviceRequest)} service
                                request <br />
                                Request Status: {serviceRequest.status} <br />
                                Created By: {serviceRequest.createdByID} <br />
                                Assigned To: {serviceRequest.assignedID}
                              </p>
                            </div>
                          )}
                        </div>
