...
export class EmailUtility {
  ...
  public async assignedServiceRequest(email: string): Promise<void> {
    return await this.transporter
      ...
      .then(() => {
        console.log(
          "Successfully sent assigned service request email to " + email,
        );
      });
  }

  public async gokuRequest(
    emails: string[],
    title: string,
    announcement: string,
    sender: string,
    ...
    return await this.transporter.sendMail({
      ...
      text:
        ...
        "\n" +
        sender +
        " sent this email to all hospital staff as part of a GOKU request: Give Ongoing Knowledge Urgently.",
    });
  }
}
