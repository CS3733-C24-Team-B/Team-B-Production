...
export abstract class GraphSearchTemplate implements searchStrategy {
  protected nodeList: MapNode[] | undefined;
  protected edgeList: MapEdge[] | undefined;
  protected graph: Graph | undefined;

  async setup() {
    this.nodeList = await createNodeList();
    this.edgeList = await createEdgeList();
    this.graph = createGraph(this.nodeList, this.edgeList);
  }
  ...
  async search(
    ...
    await this.setup();
    return this.pathFind(startNode, endNode);
  }

  abstract pathFind(startNode: string, goalNode: string): string[] | undefined;
}
...
export class AStar extends GraphSearchTemplate {
  pathFind(startNode: string, goalNode: string): string[] | undefined {
    const start = mapNodeToStar(
      this.nodeList!.find((MapNode) => MapNode.nodeID === startNode) as MapNode,
    );
    const goal = mapNodeToStar(
      this.nodeList!.find((MapNode) => MapNode.nodeID === goalNode) as MapNode,
    );
    //queue to search
    ...
    return searchNode(openList, closedList, goal, this.graph!, this.nodeList!);
  }
}

export class Dijkstra extends GraphSearchTemplate {
  pathFind(startNode: string, goalNode: string): string[] | undefined {
    //create openlist
    ...
    while (openList.length > 0) {
      openList.sort(
        (obj, obj1) =>
          (costList.get(obj) || Infinity) - (costList.get(obj1) || Infinity),
      );
      ...
      let neighbors: string[] | undefined = this.graph!.adjacencyList.get(curr);
      ...
      for (const neighbor1 of neighbors as string[]) {
        const neighbor: MapNode = findNode(this.nodeList!, neighbor1);
        if (!costList.has(curr)) {
          return undefined;
        }
        const cost =
          (costList.get(curr) as number) +
          calcCost(curr, neighbor, this.nodeList!);

        if (
          !costList.has(neighbor.nodeID) ||
          cost < (costList.get(neighbor.nodeID) || Infinity)
        ) {
...
function searchNode(
  ...
  while (openList.length > 0) {
    ...
    if (neighborsNode) {
      const neighbors = neighborsNode.map((obj) =>
        mapNodeToStar(
          nodeList.find((MapNode) => MapNode.nodeID === obj) as MapNode,
        ),
      );
      ...
      for (const neighbor of neighbors) {
        ...
        if (
          !openList.some((node) => node.nodeID === neighbor.nodeID) ||
          tentativeG < neighbor.gvalue
        ) {
          //set g to parent g+1
          ...
          if (
            nodeToFloor(neighbor) !== nodeToFloor(currentNode) &&
            (neighbor.nodeType === "STAI" || currentNode.nodeType === "STAI")
          ) {
            neighbor.gvalue += 10000;
            neighbor.floorChanges = currentNode.floorChanges + 1;
          }
          if (
            neighbor.floorChanges > 0 &&
            !canTravel(graph, neighbor, goal, nodes)
          ) {
            neighbor.gvalue += 100000;
          }
          //heuristic for current distance to goal node.
          neighbor.hvalue = Math.sqrt(
            (goal.xcoord - neighbor.xcoord) ** 2 +
              (goal.ycoord - neighbor.ycoord) ** 2 +
              ((nodeToFloor(goal) - nodeToFloor(neighbor)) * 300) ** 2,
          );
          //f is g+h
...
function calcCost(curr: string, neighbor: MapNode, nodeList: MapNode[]) {
  let weight = 1000;
  weight = Math.sqrt(
    (findNode(nodeList, curr).xcoord - neighbor.xcoord) ** 2 +
      (findNode(nodeList, curr).ycoord - neighbor.ycoord) ** 2 +
      ((nodeToFloor(mapNodeToStar(findNode(nodeList, curr))) -
        nodeToFloor(mapNodeToStar(neighbor))) *
        weight) **
        2,
  );
...
function canTravel(
  ...
  return (
    startNode.floor === endNode.floor &&
    searchNode([startNode], [], endNode, graph, nodes)?.filter(
      (obj) => findNode(nodes, obj).floor !== endNode.floor,
    ).length === 0
  );
}
/*
