...
export class AStar implements searchStrategy {
  async search(
    ...
    const start = mapNodeToStar(
      nodeList.find((MapNode) => MapNode.nodeID === startNode) as MapNode,
    );
    const goal = mapNodeToStar(
      nodeList.find((MapNode) => MapNode.nodeID === goalNode) as MapNode,
    );
  ...
  async search(startingNode: string, endingNode: string) {
    ...
    while (openList.length > 0) {
      ...
      for (const neighbor1 of neighbors as string[]) {
        ...
        const cost =
          (costList.get(curr) as number) + calcCost(curr, neighbor, nodeList);
