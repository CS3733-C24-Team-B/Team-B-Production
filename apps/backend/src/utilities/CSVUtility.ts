...
export class EmployeeCSVUtility implements CSVUtility {
    readonly headers: string[] = ["email", "firstName", "lastName", "jobTitle", "department", "birthday", "phoneNumber"];
    ...
    async upload(file: Express.Multer.File): Promise<void> {
        ...
        for (let i: number = 0; i < arrayData.length; i++) {
            ...
            try {
                ...
                console.log("Successfully added and sent invite email to " + employee[0]);
            }
            catch (error) {
...
export class NodeCSVUtility implements CSVUtility {
    readonly headers: string[] = ["nodeID", "xcoord", "ycoord", "floor", "building", "nodeType", "longName", "shortName"];
