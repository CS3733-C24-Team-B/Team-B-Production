...
router.post("/", async function (req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .send("Successfully added general service request to database");
  } catch (error) {
    console.error(error);
    return res
      .status(400)
      .send("Could not add general service request to database");
  }
});
...
router.post("/sanitation", async function (req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .send("Successfully added sanitation service request to db");
  } catch (error) {
    console.error(error);
    return res
      .status(400)
      .send("Could not add sanitation service request to db");
  }
});
...
router.post("/maintenance", async function (req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .send("Successfully added maintenance service request to db");
  } catch (error) {
    console.error(error);
    return res
      .status(400)
      .send("Could not add maintenance service request to db");
  }
});
  ...
  async function (req: Request, res: Response) {
    ...
    try {
      ...
      return res
        .status(200)
        .send(
          "Successfully added internal transportation service request to db",
        );
    } catch (error) {
      console.error(error);
      return res
        .status(400)
        .send("Could not add internal transportation service request to db");
    }
  },
);
...
router.post("/medicine", async function (req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .send("Successfully added medicine service request to db");
  } catch (error) {
    console.error(error);
...
router.post("/language", async function (req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .send("Successfully added language service request to db");
  } catch (error) {
    console.error(error);
...
router.post("/goku", async function (req: Request, res: Response) {
  ...
  const mailingList: string[] = (
    await client.employee.findMany({ select: { email: true } })
  ).map((obj) => obj.email);
  ...
  await emailUtility.gokuRequest(
    mailingList,
    gokuRequest.title,
    gokuRequest.announcement,
...
router.put("/", async function (req: Request, res: Response) {
  ...
  try {
    const currentRequest: ServiceRequest | null =
      await client.serviceRequest.findUnique({
  ...
  try {
    ...
    return res
      .status(200)
      .send(
        "Successfully updated service request with ID " +
          serviceRequest.serviceID,
      );
  } catch (error) {
    console.error(error);
    return res
      .status(400)
      .send(
        "Could not update service request with ID " + serviceRequest.serviceID,
      );
  }
});
...
router.delete("/", async function (req: Request, res: Response) {
  try {
    ...
    return res
      .status(200)
      .send(
        "Successfully deleted service request from database with ID " +
          serviceRequest,
      );
  } catch (error) {
    console.error(error);
    return res
      .status(400)
      .send("Could not delete service request from database");
  }
});
